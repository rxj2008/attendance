        ...
        <el-table-column label="加班" v-if="columns.overtime">
          <template scope="scope">
            <span class="item" v-for="item in scope.row.overtimes" :key="item.length"
                  v-html="formatTime(item.toString())">
            </span>
          </template>
        </el-table-column>
        <el-table-column label="请假" v-if="columns.leave">
          <template scope="scope">
            <span class="item" v-for="item in scope.row.leaves" :key="item.length" v-html="formatTime(item.toString())">
            </span>
          </template>
        </el-table-column>
        <el-table-column label="报销" v-if="columns.reimburse">
          <template scope="scope">
            <span class="item" v-for="item in scope.row.reimburses" :key="item.toString()"
                  v-html="formatExpense(item.toString())">
            </span>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      formatTime(value) {
        return value.split('|')[1] + '小时 <i class="time">(' + value.split('|')[0] + ')</i>'
      },
      formatExpense(value) {
        return value.split('|')[0] + ':' + value.split('|')[1] + ' <i class="time">(' + value.split('|')[2] + ')</i>'
        // return value + '小时 <i class="time">(' + value.split('|')[0] + ')</i>'
      },
      query() {
        this.setColumns(this.type)
        switch (this.type) {
          case OVERTIME:
            this.attendances = this.allAttendances.filter(item => {
              return item.overtimes.length
            })
            break
          case LEAVE:
            this.attendances = this.allAttendances.filter(item => {
      ...
      setRowClass(data) {
        ...
        return data.isWorkday ? '' : 'background: rgba(32, 160, 255, 0.08);'
      }
    },
    mounted() {
